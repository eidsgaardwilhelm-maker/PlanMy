...
const Input = ({value,onChange,placeholder,type="text",style:s={},multiline,rows=3,autoFocus,min,max}) => {
  const {dark}=useTheme(); const t=tk(dark);
  const base={width:"100%",padding:"9px 12px",borderRadius:t.rSm,border:`1px solid ${t.border}`,background:t.surfaceAlt,color:t.text,fontSize:13,outline:"none",transition:"border-color .15s",resize:"none",...s};
...
const Badge = ({count,color="rose"}) => {
  ...
  const colors={rose:t.rose,accent:t.accent};
  return <span style={{minWidth:17,height:17,borderRadius:9,background:colors[color],color:"#fff",fontSize:10,fontWeight:700,display:"inline-flex",alignItems:"center",justifyContent:"center",padding:"0 4px"}}>{count>99?"99+":count}</span>;
};
  ...
  const submit = () => {
    ...
    setTimeout(()=>{
      ...
      if(mode==="signup"){
        ...
        if(users[email.toLowerCase()]) return setErr("An account with this email already exists.");
        const user={id:genId(),name:name.trim(),email:email.toLowerCase(),password:pw,createdAt:new Date().toISOString()};
        users[user.email]=user; db.set("users",users); db.set("session",user.id); onLogin(user);
      } else {
        const user=users[email.toLowerCase()];
        if(!user||user.password!==pw) return setErr("Invalid email or password.");
        db.set("session",user.id); onLogin(user);
      }
    },400);
  };
...
const TaskForm = ({initial={},onSave,onCancel,currentUser}) => {
  ...
  const [useTime,setUseTime]=useState(!!initial.dueTime);
  ...
  const addCollab = () => {
    ...
    if(!em) return;
    const users=db.get("users",{});
    if(!users[em]) return setErr("No account found with this email.");
    if(em===currentUser.email) return setErr("You can't add yourself.");
    if(collabs.find(c=>c.email===em)) return setErr("Already added.");
    setCollabs([...collabs,{id:users[em].id,email:em,name:users[em].name,status:"pending"}]);
    setCollabEmail("");
  };
  ...
  const handleFiles = e => {
    Array.from(e.target.files).forEach(f=>{
      ...
      reader.onload=ev=>setFiles(prev=>[...prev,{id:genId(),name:f.name,size:f.size,type:f.type,data:ev.target.result,uploadedAt:new Date().toISOString()}]);
  ...
  const save = () => {
    ...
    onSave({title:title.trim(),description:desc,dueDate:due,dueTime:useTime?time:"",collaborators:collabs,files});
  };
  ...
  return (
    <div>
      ...
      <Field label="Due date">
        <Input value={due} onChange={setDue} type="date" min={localDate}/>
      </Field>
      {due && (
        <Field label="Due time (optional)">
          <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
            ...
            {useTime && (
              <Input value={time||localTime} onChange={setTime} type="time" style={{width:130}}/>
            )}
          </div>
          {useTime && <p style={{fontSize:11,color:t.textFaint,marginTop:4}}>Uses your device's local timezone</p>}
        </Field>
      )}
      <Field label="Collaborators">
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          <Input value={collabEmail} onChange={setCollabEmail} placeholder="Email address" style={{flex:1}}/>
          <Btn onClick={addCollab} variant="subtle" size="sm">Add</Btn>
        </div>
        {collabs.map(c=>(
          <div key={c.email} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 10px",background:t.surfaceAlt,borderRadius:t.rSm,marginBottom:4,fontSize:13}}>
            ...
            <span onClick={()=>setCollabs(collabs.filter(x=>x.email!==c.email))} style={{cursor:"pointer",color:t.rose,fontSize:18,lineHeight:1}}>×</span>
          </div>
        ))}
      </Field>
...
const TaskCard = ({task,onToggle,onDelete,onEdit,projectName,showProject}) => {
  ...
  const over = isOverdue(task.dueDate, task.dueTime) && !task.completed;
  const dToday = isDueToday(task.dueDate) && !task.completed;
  const accepted=(task.collaborators||[]).filter(c=>c.status==="accepted");
  const hasFiles=(task.files||[]).length>0;

  return (
    <div className="fade-in" style={{background:t.surface,borderRadius:t.r,border:`1px solid ${t.border}`,marginBottom:8,opacity:task.completed?.6:1,transition:"opacity .2s"}}>
      <div style={{padding:"12px 14px",display:"flex",gap:12,alignItems:"flex-start",cursor:"pointer"}} onClick={()=>setExpanded(!expanded)}>
        <div onClick={e=>{e.stopPropagation();onToggle(task.id);}} style={{width:20,height:20,borderRadius:6,border:`2px solid ${task.completed?t.accent:t.border}`,background:task.completed?t.accent:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1,cursor:"pointer",transition:"all .15s"}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
            ...
            <div style={{display:"flex",gap:4,flexShrink:0,alignItems:"center"}}>
              ...
              {hasFiles&&!task.completed&&<span title="Has attachments" style={{fontSize:13}}>📎</span>}
          ...
          <div style={{display:"flex",gap:6,marginTop:5,flexWrap:"wrap",alignItems:"center"}}>
            {task.dueDate&&(
              <span style={{fontSize:11,fontWeight:600,color:over?t.rose:dToday?t.amber:t.textMuted,background:over?t.roseBg:dToday?t.amberBg:t.surfaceAlt,padding:"2px 7px",borderRadius:20}}>
                {over?"⚠ ":dToday?"● ":""}{fmtDateTime(task.dueDate, task.dueTime)}
              </span>
            )}
        ...
        <div style={{padding:"0 14px 14px 46px",borderTop:`1px solid ${t.border}`}}>
          ...
          {(task.files||[]).length>0&&!task.completed&&(
            <div style={{marginBottom:10}}>
              <p style={{fontSize:11,fontWeight:600,color:t.textFaint,textTransform:"uppercase",letterSpacing:.5,marginBottom:6}}>Attachments</p>
              {task.files.map(f=>(
          ...
          {task.completed&&(task.files||[]).length>0&&<p style={{fontSize:12,color:t.textFaint,fontStyle:"italic",marginBottom:8}}>Files permanently deleted on completion.</p>}
          {accepted.length>0&&(
            <div style={{marginBottom:10}}>
              <p style={{fontSize:11,fontWeight:600,color:t.textFaint,textTransform:"uppercase",letterSpacing:.5,marginBottom:6}}>Collaborators</p>
          ...
          {!task.completed&&(
            <div style={{display:"flex",gap:6,marginTop:8}}>
              ...
              <Btn size="sm" variant="ghost" onClick={()=>onDelete(task.id)} style={{color:t.rose,borderColor:t.rose}}>Delete</Btn>
            </div>
          )}
        </div>
      )}
...
const HomeTab = ({currentUser,tasks,projects,onAddTask,onToggleTask,onDeleteTask,onEditTask}) => {
  ...
  const visible=tasks.filter(tk=>
    tk.ownerId===currentUser.id ||
    (tk.collaborators||[]).some(c=>c.id===currentUser.id&&c.status==="accepted")
  );

  const dueToday=visible.filter(tk=>isDueToday(tk.dueDate)&&!tk.completed);
  const completedToday=visible.filter(tk=>tk.completed&&tk.completedAt&&tk.completedAt.slice(0,10)===todayStr());
  const overdue=visible.filter(tk=>isOverdue(tk.dueDate,tk.dueTime)&&!tk.completed);

  const sorted=[...visible].sort((a,b)=>{
    if(a.completed&&!b.completed) return 1;
    if(!a.completed&&b.completed) return -1;
    if(isOverdue(a.dueDate,a.dueTime)&&!isOverdue(b.dueDate,b.dueTime)) return -1;
    if(!isOverdue(a.dueDate,a.dueTime)&&isOverdue(b.dueDate,b.dueTime)) return 1;
    if(isDueToday(a.dueDate)&&!isDueToday(b.dueDate)) return -1;
    if(!isDueToday(a.dueDate)&&isDueToday(b.dueDate)) return 1;
    const da=(a.dueDate||"9999")+(a.dueTime||"");
    const db2=(b.dueDate||"9999")+(b.dueTime||"");
    return da<db2?-1:1;
  });
    ...
    <div style={{padding:"18px 15px"}}>
      <div style={{background:t.surface,borderRadius:t.rLg,padding:20,marginBottom:18,boxShadow:t.shadow}}>
        ...
        <div style={{height:5,background:t.surfaceAlt,borderRadius:3,overflow:"hidden"}}>
          <div style={{height:"100%",width:`${prog*100}%`,background:t.accent,borderRadius:3,transition:"width .5s cubic-bezier(.16,1,.3,1)"}}/>
        </div>
      </div>
      ...
      {sorted.map(tk=>(
        <TaskCard key={tk.id} task={tk} onToggle={onToggleTask} onDelete={onDeleteTask}
          ...
          projectName={getProjectName(tk.projectId)}/>
      ))}
...
const ProjectEditModal = ({open,onClose,project,currentUser,onSave,onSendNotif}) => {
  ...
  const addMember = () => {
    ...
    const users=db.get("users",{});
    if(!users[em]) return setErr("No account found with this email.");
    if(em===currentUser.email) return setErr("You're the owner already.");
    if(members.find(m=>m.email===em)) return setErr("Already added.");
    const newM={id:users[em].id,email:em,name:users[em].name,status:"pending"};
    ...
    <Modal open={open} onClose={onClose} title="Edit Project">
      ...
      <Field label="Members">
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          ...
          <Btn onClick={addMember} variant="subtle" size="sm">Invite</Btn>
        </div>
        ...
        <div style={{background:t.surfaceAlt,borderRadius:t.r,padding:10,marginTop:4}}>
          <div style={{padding:"4px 8px",fontSize:13,marginBottom:4,display:"flex",gap:8,alignItems:"center"}}>
            <span>👑 {currentUser.name}</span>
            <span style={{fontSize:11,color:t.textMuted}}>(owner)</span>
          </div>
          {members.map(m=>(
            <div key={m.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 8px",fontSize:13,borderTop:`1px solid ${t.border}`}}>
              ...
              <span onClick={()=>setMembers(members.filter(x=>x.id!==m.id))} style={{cursor:"pointer",color:t.rose,fontSize:18,marginLeft:8}}>×</span>
            </div>
          ))}
          {members.length===0&&<p style={{fontSize:12,color:t.textFaint,textAlign:"center",padding:"4px 0"}}>No members yet</p>}
        </div>
      </Field>
...
const ProjectsTab = ({currentUser,projects,tasks,onCreateProject,onUpdateProject,onArchiveProject,onRestoreProject,onAddProjectTask,onToggleTask,onDeleteTask,onEditTask,onSendNotif}) => {
  ...
  const addMember = () => {
    ...
    if(!em) return;
    const users=db.get("users",{});
    if(!users[em]) return setMemberErr("No account found.");
    if(em===currentUser.email) return setMemberErr("You're already the owner.");
    if(members.find(m=>m.email===em)) return setMemberErr("Already added.");
    setMembers([...members,{id:users[em].id,email:em,name:users[em].name,status:"pending"}]);
    setMemberEmail("");
  };
  ...
  const projTasks=selected?tasks.filter(tk=>tk.projectId===selected.id):[];
        ...
        <div>
          <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:18}}>
            ...
            <div style={{display:"flex",gap:6,flexShrink:0}}>
              {selected.ownerId===currentUser.id&&!selected.archived&&(
                <>
              ...
              {selected.archived&&selected.ownerId===currentUser.id&&(
          ...
          <div style={{background:t.surface,borderRadius:t.r,padding:14,marginBottom:14,border:`1px solid ${t.border}`}}>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              <span style={{fontSize:12,padding:"3px 10px",background:t.accentLight,color:t.accent,borderRadius:20,fontWeight:600}}>👑 {currentUser.name}</span>
          ...
          <Modal open={showTaskForm||!!editingTask} onClose={()=>{setShowTaskForm(false);setEditingTask(null);}} title={editingTask?"Edit Task":"New Project Task"}>
            <TaskForm initial={editingTask||{}} onSave={data=>{
              ...
              else{onAddProjectTask({...data,projectId:selected.id});setShowTaskForm(false);}
        ...
        <div>
            ...
            return (
              <div key={p.id} onClick={()=>setSelected(p)} className="fade-in" style={{background:t.surface,borderRadius:t.r,border:`1px solid ${t.border}`,padding:16,marginBottom:10,cursor:"pointer"}}>
                ...
                <div style={{height:4,background:t.surfaceAlt,borderRadius:2,overflow:"hidden"}}>
                  <div style={{height:"100%",width:`${prog*100}%`,background:t.accent,borderRadius:2,transition:"width .4s"}}/>
                </div>
          ...
          {archived.length>0&&(
            <div style={{marginTop:22}}>
              <button onClick={()=>setShowArchived(!showArchived)} style={{background:"none",border:"none",cursor:"pointer",color:t.textMuted,fontSize:13,fontWeight:600,display:"flex",gap:6,alignItems:"center"}}>
                {showArchived?"▼":"▶"} Archived ({archived.length})
              </button>
              {showArchived&&archived.map(p=>(
                <div key={p.id} onClick={()=>setSelected(p)} style={{background:t.surface,borderRadius:t.r,border:`1px solid ${t.border}`,padding:14,marginTop:8,cursor:"pointer",opacity:.6}}>
          ...
          <Modal open={showCreate} onClose={()=>setShowCreate(false)} title="New Project">
            ...
            <Field label="Invite members">
              <div style={{display:"flex",gap:8,marginBottom:8}}>
                <Input value={memberEmail} onChange={setMemberEmail} placeholder="Email" style={{flex:1}}/>
                <Btn onClick={addMember} variant="subtle" size="sm">Add</Btn>
              </div>
              {memberErr&&<p style={{color:t.rose,fontSize:12,marginBottom:6}}>{memberErr}</p>}
              {members.map(m=>(
                <div key={m.id} style={{display:"flex",justifyContent:"space-between",padding:"5px 10px",background:t.surfaceAlt,borderRadius:t.rSm,fontSize:12,marginBottom:4}}>
                  {m.name} <span onClick={()=>setMembers(members.filter(x=>x.id!==m.id))} style={{cursor:"pointer",color:t.rose}}>×</span>
                </div>
              ))}
            </Field>
            <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
              <Btn onClick={()=>setShowCreate(false)} variant="ghost">Cancel</Btn>
              <Btn onClick={create} variant="primary" disabled={!name.trim()}>Create</Btn>
            </div>
          </Modal>
        </div>
      )}
...
const NotificationsTab = ({currentUser,notifications,onAccept,onDecline}) => {
  const {dark}=useTheme(); const t=tk(dark);
  const mine=notifications.filter(n=>n.toId===currentUser.id&&n.status==="pending");
  const past=notifications.filter(n=>n.toId===currentUser.id&&n.status!=="pending");
  return (
    <div style={{padding:"18px 15px"}}>
      <h2 style={{fontWeight:700,fontSize:18,marginBottom:18}}>Notifications</h2>
      {mine.length===0&&past.length===0&&(
        <div style={{textAlign:"center",padding:"40px 20px",color:t.textMuted}}>
          <div style={{fontSize:32,marginBottom:10}}>🔔</div>
      ...
      {mine.length>0&&(
        <div style={{marginBottom:22}}>
          <p style={{fontSize:11,fontWeight:600,color:t.textFaint,textTransform:"uppercase",letterSpacing:.5,marginBottom:10}}>Pending</p>
          {mine.map(n=>(
            <div key={n.id} className="fade-in" style={{background:t.surface,borderRadius:t.r,border:`1px solid ${t.border}`,padding:16,marginBottom:8}}>
              <div style={{display:"flex",gap:10,alignItems:"flex-start",marginBottom:12}}>
                <span style={{fontSize:20}}>{n.type==="project"?"◇":"✦"}</span>
                <div style={{flex:1}}>
                  <p style={{fontWeight:600,fontSize:14,marginBottom:3}}>{n.type==="project"?"Project invitation":"Task collaboration"}</p>
                  <p style={{fontSize:13,color:t.textMuted}}><strong style={{color:t.text}}>{n.fromName}</strong> invited you to <strong style={{color:t.text}}>{n.resourceName}</strong></p>
                </div>
              </div>
              <div style={{display:"flex",gap:8}}>
                <Btn size="sm" variant="primary" onClick={()=>onAccept(n)}>Accept</Btn>
                <Btn size="sm" variant="ghost" onClick={()=>onDecline(n)} style={{color:t.rose,borderColor:t.rose}}>Decline</Btn>
              </div>
            </div>
          ))}
        </div>
      ...
      {past.length>0&&(
        <div>
          <p style={{fontSize:11,fontWeight:600,color:t.textFaint,textTransform:"uppercase",letterSpacing:.5,marginBottom:10}}>History</p>
          {past.map(n=>(
            <div key={n.id} style={{background:t.surface,borderRadius:t.r,border:`1px solid ${t.border}`,padding:"12px 16px",marginBottom:6,opacity:.65,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                <p style={{fontSize:13,fontWeight:600}}>{n.resourceName}</p>
                <p style={{fontSize:12,color:t.textMuted}}>from {n.fromName}</p>
              </div>
              <Tag label={n.status} color={n.status==="accepted"?"green":"rose"}/>
            </div>
          ))}
        </div>
      )}
...
const SettingsTab = ({currentUser,onLogout,onDeleteAccount,toggleDark,dark}) => {
  ...
  const confirmDelete = () => {
    ...
    if(!user||user.password!==pw) return setErr("Incorrect password.");
    onDeleteAccount();
  };
  ...
  return (
    <div style={{padding:"18px 15px"}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:24}}>
        ...
        <h2 style={{fontFamily:"'DM Serif Display',serif",fontWeight:400,fontSize:22}}>PlanMy</h2>
      </div>

      <div style={{background:t.surface,borderRadius:t.rLg,border:`1px solid ${t.border}`,overflow:"hidden",marginBottom:14}}>
        <div style={{padding:"16px 18px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <div style={{width:42,height:42,borderRadius:"50%",background:t.accentLight,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:16,color:t.accent}}>
      ...
      <div style={{background:t.surface,borderRadius:t.rLg,border:`1px solid ${t.border}`,overflow:"hidden",marginBottom:14}}>
        <div style={{padding:"16px 18px",borderBottom:`1px solid ${t.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <p style={{fontWeight:600,fontSize:14}}>Appearance</p>
            <p style={{fontSize:12,color:t.textMuted}}>{dark?"Dark mode":"Light mode"}</p>
          </div>
          <div onClick={toggleDark} style={{width:46,height:26,borderRadius:13,background:dark?t.accent:t.surfaceAlt,border:`1px solid ${t.border}`,cursor:"pointer",position:"relative",transition:"background .2s"}}>
            <div style={{position:"absolute",top:3,left:dark?22:3,width:18,height:18,borderRadius:"50%",background:dark?"#fff":t.textMuted,transition:"left .2s"}}/>
          </div>
        </div>
        <div style={{padding:"16px 18px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <p style={{fontWeight:600,fontSize:14}}>Sign out</p>
            <p style={{fontSize:12,color:t.textMuted}}>End your current session</p>
          </div>
          <Btn size="sm" variant="ghost" onClick={onLogout}>Logout</Btn>
        </div>
      </div>

      <div style={{background:t.surface,borderRadius:t.rLg,border:`1px solid ${t.roseBg}`,overflow:"hidden"}}>
        <div style={{padding:"16px 18px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <p style={{fontWeight:600,fontSize:14,color:t.rose}}>Delete Account</p>
            <p style={{fontSize:12,color:t.textMuted}}>Permanently remove all your data</p>
          </div>
          <Btn size="sm" variant="danger" onClick={()=>setShowDelete(true)}>Delete</Btn>
        </div>
      </div>

      <Modal open={showDelete} onClose={()=>{setShowDelete(false);setPw("");setErr("");}} title="Delete Account" width={400}>
        <div style={{marginBottom:14,padding:"12px 14px",background:t.roseBg,borderRadius:t.rSm,fontSize:13,color:t.rose}}>
          ⚠ This permanently deletes your account, all tasks, files, and removes you from all projects. Cannot be undone.
        </div>
        <Field label="Confirm password"><Input value={pw} onChange={setPw} type="password" placeholder="Enter your password" autoFocus/></Field>
        {err&&<p style={{color:t.rose,fontSize:13,marginBottom:12}}>{err}</p>}
        <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
          <Btn onClick={()=>{setShowDelete(false);setPw("");setErr("");}} variant="ghost">Cancel</Btn>
          <Btn onClick={confirmDelete} variant="danger">Delete forever</Btn>
        </div>
      </Modal>
    </div>
  );
...
const BottomNav = ({tab,setTab,notifCount,dark}) => {
  ...
  const tabs=[
    {id:"home",label:"Home",icon:(a)=>(
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={a?t.accent:t.textFaint} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/>
      </svg>
    )},
    {id:"projects",label:"Projects",icon:(a)=>(
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={a?t.accent:t.textFaint} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div style={{position:"fixed",bottom:0,left:0,right:0,background:t.surface,borderTop:`1px solid ${t.border}`,display:"flex",zIndex:100,maxWidth:480,margin:"0 auto",paddingBottom:"env(safe-area-inset-bottom)"}}>
      {tabs.map(tb=>{
        const active=tab===tb.id;
        return (
          <button key={tb.id} onClick={()=>setTab(tb.id)} style={{flex:1,padding:"10px 8px 12px",background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,position:"relative"}}>
            {tb.icon(active)}
            <span style={{fontSize:10,fontWeight:600,color:active?t.accent:t.textFaint,transition:"color .15s"}}>{tb.label}</span>
...
export default function App() {
  const [dark,setDark]=useState(()=>db.get("darkMode",false));
  const [currentUser,setCurrentUser]=useState(()=>{
    const sid=db.get("session");
    if(!sid) return null;
    const users=db.get("users",{});
    return Object.values(users).find(u=>u.id===sid)||null;
  });
  const [tab,setTab]=useState("home");
  const [tasks,setTasks]=useState(()=>db.get("tasks",[]));
  const [projects,setProjects]=useState(()=>db.get("projects",[]));
  const [notifications,setNotifications]=useState(()=>db.get("notifications",[]));

  const save=(key,val,setter)=>{setter(val);db.set(key,val);};
  const pendingNotifs=notifications.filter(n=>n.toId===currentUser?.id&&n.status==="pending").length;
  const toggleDark=()=>{const n=!dark;setDark(n);db.set("darkMode",n);};

  // ── Live updates: sync from localStorage (other tabs) + poll every 3s
  const [lastSync,setLastSync]=useState(Date.now());
  const syncFromStorage = useCallback(() => {
    const t2=db.get("tasks",[]);
    const p2=db.get("projects",[]);
    const n2=db.get("notifications",[]);
    setTasks(prev=>JSON.stringify(prev)!==JSON.stringify(t2)?t2:prev);
    setProjects(prev=>JSON.stringify(prev)!==JSON.stringify(p2)?p2:prev);
    setNotifications(prev=>JSON.stringify(prev)!==JSON.stringify(n2)?n2:prev);
    setLastSync(Date.now());
  }, []);

  useEffect(()=>{
    const onStorage=e=>{if(e.key==="tasks"||e.key==="projects"||e.key==="notifications") syncFromStorage();};
    window.addEventListener("storage",onStorage);
    const interval=setInterval(syncFromStorage,3000);
    return ()=>{window.removeEventListener("storage",onStorage);clearInterval(interval);};
  },[syncFromStorage]);

  const sendNotif=({type,toId,resourceId,resourceName})=>{
    if(!currentUser||toId===currentUser.id) return;
    const already=notifications.find(n=>n.toId===toId&&n.resourceId===resourceId&&n.status==="pending");
    if(already) return;
    const n={id:genId(),type,toId,fromId:currentUser.id,fromName:currentUser.name,resourceId,resourceName,status:"pending",createdAt:new Date().toISOString()};
    const next=[...notifications,n];
    save("notifications",next,setNotifications);
  };

  const addTask=(data)=>{
    if(!currentUser) return;
    const task={id:genId(),ownerId:currentUser.id,createdAt:new Date().toISOString(),completed:false,...data};
    const next=[...tasks,task];
    save("tasks",next,setTasks);
    (data.collaborators||[]).forEach(c=>sendNotif({type:"task",toId:c.id,resourceId:task.id,resourceName:task.title}));
  };

  const editTask=(updated)=>save("tasks",tasks.map(tk=>tk.id===updated.id?updated:tk),setTasks);

  const toggleTask=(id)=>{
    save("tasks",tasks.map(tk=>{
      if(tk.id!==id) return tk;
      return tk.completed?{...tk,completed:false,completedAt:null}:{...tk,completed:true,completedAt:new Date().toISOString(),files:[]};
    }),setTasks);
  };

  const deleteTask=(id)=>save("tasks",tasks.filter(tk=>tk.id!==id),setTasks);

  const createProject=(data)=>{
    if(!currentUser) return;
    const proj={id:genId(),ownerId:currentUser.id,createdAt:new Date().toISOString(),archived:false,...data};
    const next=[...projects,proj];
    save("projects",next,setProjects);
    (data.members||[]).forEach(m=>sendNotif({type:"project",toId:m.id,resourceId:proj.id,resourceName:proj.name}));
  };

  const updateProject=(updated)=>save("projects",projects.map(p=>p.id===updated.id?updated:p),setProjects);
  const archiveProject=(id)=>save("projects",projects.map(p=>p.id===id?{...p,archived:true}:p),setProjects);
  const restoreProject=(id)=>save("projects",projects.map(p=>p.id===id?{...p,archived:false}:p),setProjects);

  const acceptNotif=(notif)=>{
    save("notifications",notifications.map(n=>n.id===notif.id?{...n,status:"accepted"}:n),setNotifications);
    if(notif.type==="task"){
      save("tasks",tasks.map(tk=>{
        if(tk.id!==notif.resourceId) return tk;
        return{...tk,collaborators:(tk.collaborators||[]).map(c=>c.id===currentUser.id?{...c,status:"accepted"}:c)};
      }),setTasks);
    } else {
      save("projects",projects.map(p=>{
        if(p.id!==notif.resourceId) return p;
        return{...p,members:(p.members||[]).map(m=>m.id===currentUser.id?{...m,status:"accepted"}:m)};
      }),setProjects);
    }
  };

  const declineNotif=(notif)=>save("notifications",notifications.map(n=>n.id===notif.id?{...n,status:"declined"}:n),setNotifications);
  const handleLogout=()=>{db.del("session");setCurrentUser(null);setTab("home");};

  const handleDeleteAccount=()=>{
    const uid=currentUser.id;
    const users=db.get("users",{});
    delete users[currentUser.email]; db.set("users",users);
    save("tasks",tasks.filter(tk=>tk.ownerId!==uid).map(tk=>({...tk,collaborators:(tk.collaborators||[]).filter(c=>c.id!==uid)})),setTasks);
    save("projects",projects.filter(p=>p.ownerId!==uid).map(p=>({...p,members:(p.members||[]).filter(m=>m.id!==uid)})),setProjects);
    save("notifications",notifications.filter(n=>n.toId!==uid&&n.fromId!==uid),setNotifications);
    db.del("session"); setCurrentUser(null); setTab("home");
  };
      ...
      {!currentUser ? (
        ...
        <div style={{maxWidth:480,margin:"0 auto",minHeight:"100vh",background:t.bg,position:"relative"}}>
          <div style={{position:"sticky",top:0,zIndex:50,background:t.surface,borderBottom:`1px solid ${t.border}`,padding:"11px 16px",display:"flex",alignItems:"center",gap:10}}>
            ...
            <div title="Live — auto-syncing" style={{width:7,height:7,borderRadius:"50%",background:t.green,flexShrink:0,boxShadow:"0 0 0 2px rgba(74,143,72,.25)",animation:"pulse 2s infinite"}}/>
          </div>
